...
const KEYWORDS = ["", "flying", "haste", "trample", "lifelink", "deathtouch", "vigilance", "indestructible", "flash", "first strike", "double strike", "defender"];
...
export function CardFilterBar({ onQuery, opts, setOpts, interpreted, queryError, compact = false, autoFocus = false }: CardFilterBarProps) {
  ...
  const [colorType, setColorType] = useState<"color" | "identity">("color");
  ...
  const effective = useMemo(() => {
    ...
    if (colors.size > 0) {
      const prefix = colorType === "identity" 
        ? (colorMode === "exact" ? "id=" : "id:")
        : (colorMode === "exact" ? "c=" : "c:");
      parts.push(prefix + [...colors].join("").toLowerCase());
    }
    ...
    return parts.join(" ");
  }, [text, colors, colorMode, colorType, multi, colorless, rarity, legal, sets, cmcMin, cmcMax, cardType, keyword]);
  ...
  const anyActive = colors.size > 0 || multi || colorless || !!rarity || !!legal || sets.size > 0 || !!cmcMin || !!cmcMax || !!cardType || !!keyword;
    ...
    <div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className={`chip flex items-center gap-1 ${expanded || anyActive ? "border-table-accent text-table-accentSoft" : "opacity-75"}`}
        >
          <span>⚙️ Filters</span>
          {anyActive && <span className="h-1.5 w-1.5 rounded-full bg-table-accent animate-pulse" />}
        </button>
      </div>

      {/* Filter row */}
      {expanded && (
        <div className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-2 border-t border-table-border/40 pt-2 animate-fade-in">
          <div className="flex items-center gap-1">
            <select
              className="input !py-0.5 !px-1 text-xs"
              value={colorType}
              onChange={(e) => setColorType(e.target.value as "color" | "identity")}
            >
          ...
          <select className="input !py-1" value={cardType} onChange={(e) => setCardType(e.target.value)}>
            {TYPES.map((t) => (
              <option key={t} value={t}>
                {t ? `Type: ${t.charAt(0).toUpperCase() + t.slice(1)}` : "Any type"}
              </option>
            ))}
          </select>

          <select className="input !py-1" value={keyword} onChange={(e) => setKeyword(e.target.value)}>
            {KEYWORDS.map((k) => (
              <option key={k} value={k}>
                {k ? `Keyword: ${k.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}` : "Any keyword"}
              </option>
            ))}
          </select>
